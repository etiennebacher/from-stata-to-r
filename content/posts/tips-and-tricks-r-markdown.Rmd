...
In addition to `Global.Rmd` (which contains YAML and setup chunks), I used two `.tex` files: `preamble.tex` is where I put all the LaTeX commands and packages I used, and `titlepage.tex` to make my custom titlepage, acknowledgments and summary before the table of contents. 
...
## Titlepage, acknowledgements and summary
...
The next step was to create two pages containing the acknowledgements and the summary. This was easily done in LaTeX, and this time I could use `\pagebreak` at the end of the acknowledgements to create a new page for the summary. I also put another `\pagebreak` to finish `titlepage.tex`, so that TOC, LOF and LOT (created in the YAML) could start on a new page.
